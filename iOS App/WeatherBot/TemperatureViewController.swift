//
//  GraphViewController.swift
//  Lemo
//
//  Created by Finn Gaida on 11.06.16.
//  Copyright © 2016 Finn Gaida. All rights reserved.
//

import UIKit
import Charts

class TemperatureViewController: UIViewController {
    
    var dataSet:DataSet!
    var rawDataSet:[Double] = []
    var chart:TemperatureView!
    var timer:NSTimer!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        self.title = "Graph"
        
        timer = NSTimer(timeInterval: 2.0, target: self, selector: #selector(TemperatureViewController.reload), userInfo: nil, repeats: true)
        NSRunLoop.currentRunLoop().addTimer(timer, forMode: NSRunLoopCommonModes)
        reload()
    }
    
    func reload() {
        do {
            dataSet = try Manager.sharedManager.getAllData()
            
            chart = TemperatureView(frame: CGRectMake(5, 70, self.view.frame.width - 10, self.view.frame.height - 130))
            rawDataSet = dataSet.data.map({ $0.temperature })
            chart.setDataSet(rawDataSet)
            chart.chart?.zoom(CGFloat(dataSet.data.count) / 15, scaleY: 1.0, x: chart.frame.width * 14, y: 0)
            self.view.addSubview(chart)
            
            let loader = UIActivityIndicatorView(activityIndicatorStyle: .White)
            loader.center = CGPointMake(self.view.center.x, 100)
            loader.startAnimating()
            self.view.addSubview(loader)

        } catch let e {
            log.error("unwrap error: \(e)")
            self.navigationController?.popToRootViewControllerAnimated(true)
        }
        
        //do {
            //let data = try Manager.sharedManager.getAllData() // getLatestData() {
            ////self.rawDataSet.append(data.temperature)
            ////chart.chart?.data?.dataSets.first?.addEntry(ChartDataEntry(value: data.temperature, xIndex: self.rawDataSet.count - 1))
            ////chart.chart?.data?.notifyDataChanged()
            ////chart.chart?.notifyDataSetChanged()
            ////chart.chart?.data = nil
            ////chart.setDataSet(self.rawDataSet)
            
            //rawDataSet = dataSet.data.map({ $0.temperature })
            //chart.setDataSet(rawDataSet)
            //chart.chart?.zoom(CGFloat(dataSet.data.count) / 15, scaleY: 1.0, x: chart.frame.width * 14, y: 0)
            //chart.todayText = String(format: "%.2f°C", data.data.last!.temperature)
            //log.verbose("updating chart to count: \(self.rawDataSet.count)")
            ////} else {
            ////log.debug("no new data found")
            ////}
        //} catch let e {
            //log.error("An error occurred while unwrapping latest data: \(e)")
            //let alert = UIAlertController(title: "Oh nein!", message: "Es gab einen Fehler beim Laden der Daten, bitte versichere dich, dass mindestens eine Wetterstation online ist und versuche es dann nochmal!", preferredStyle: .Alert)
            //alert.addAction(UIAlertAction(title: "Erneut versuchen", style: .Default, handler: { (action) in
                //self.reload()
            //}))
            //alert.addAction(UIAlertAction(title: "Schließen", style: .Default, handler: nil))
            //self.presentViewController(alert, animated: true, completion: nil)
            //return
        //}
    }
    
    override func viewWillDisappear(animated: Bool) {
        //timer.invalidate()
    }
    
    override func didReceiveMemoryWarning() {
        super.didReceiveMemoryWarning()
        // Dispose of any resources that can be recreated.
    }
    
    
    /*
     // MARK: - Navigation
     
     // In a storyboard-based application, you will often want to do a little preparation before navigation
     override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
     // Get the new view controller using segue.destinationViewController.
     // Pass the selected object to the new view controller.
     }
     */
    
}